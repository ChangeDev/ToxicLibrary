...
module.exports = class RewardsCommand extends Command {
    ...
    async run(message) {
        ...
        if (rewardsKeys.length == 0) {
            return await message.channel.send({embed: ToxicUtils.generateInfoEmbed(message, 'There are no rewards in this guild.')});
        }
        ...
        await message.channel.send({embed: ToxicUtils.generateInfoEmbed(message, 'Rewards', text)});
    }
};
