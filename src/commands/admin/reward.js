    ...
    async run(message, {option, role, level}) {
        if (option == 'add') {
            if (await this.client.rewards.addGuildReward(message.guild, role, level)) {
                await message.channel.send({embed: ToxicUtils.generateSuccessEmbed(message, 'Reward added!', `__Role__: ${role}\n__Level__: ${level}`)});
            } else {
                return await message.channel.send({embed: ToxicUtils.generateFailEmbed(message, 'Role is already a reward!')});
            }
        } else {
            if (await this.client.rewards.removeGuildReward(message.guild, role)) {
                await message.channel.send({embed: ToxicUtils.generateSuccessEmbed(message, 'Reward removed!')});
            } else {
                return await message.channel.send({embed: ToxicUtils.generateFailEmbed(message, 'Role is not a reward!')});
            }
        }
    }
};
